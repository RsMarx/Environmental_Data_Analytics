...
#could put the legend on the insdie of the graph using position and justification commands: legend.position = c(1,1), legend.justification - c(1,1)
...
theme_set(mytheme) #will use the theme throughout the remainder of your session. 
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 0, ymax = 60, fill = "green") +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), #put recs as starting and ending after the data becuse otherwise there will be white space around it 
            ymin = 60, ymax = 100, fill = "pink") +
  geom_point() +
  geom_text(x = as.Date("2019-01-01"), y = 50, label = "good", hjust = 1, fontface = "bold") +
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "bad", hjust = 1, fontface = "italic") +
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), #why does it start in Dec 2016
    date_breaks = "1 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + #[]is a subscript; superscript is a carrot 
  theme(axis.text.x = element_text(angle = 60,  hjust = 1)) #hjust of 1 moves things to the left. vjust moves things up and down. If you put text on your plot it will always plot them centered unless you say otherwise. 
print(O3plot7)  
...
NvsP2 <-
  ...
  xlab(expression(Total_P_Grams)) #Doesn't seem that you can use spaces; Could use labs: labs(x = "Total P, micro g/L", y = "Total N, micro g/L", fill = "Depth, m", shape = "Lake"
  ylab(expression(TN_UG)) + #why didn't it change?
