...
summary(NTL.phys.data.PeterPaul$comments) #ask for summary of specific column 
...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake") #filter select rows; select selects columns 
...
#drop.levels should read that we only have Peter and Paul Lake associated wtih a column 

# Remove columns that are not of interest for analysis
NTL.phys.data.PeterPaul.skinny <- select(NTL.phys.data.PeterPaul, 
                                         lakename, daynum, year4, sampledate:irradianceDeck) #tell it which columns we want to keep. Could put a minuns sign in front of all of them if you wanted it to remove them. 
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) #takes 5 columns and gathers in to one. Will take maximum number in original data set  
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #remove has associated with concentration 
...
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) #indicate which column you wnat to spread by. In this case the nutrient column. 

# Split components of cells into multiple columns
# Opposite of 'separate' is 'unite'
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.skinny, sampledate, c("Y", "m", "d")) #will store as a character class rather than a date 
...
```{r}
#install.packages(lubridate) #Good package if you're using data from different people 
library(lubridate)
...
NTL.phys.nutrient.data.PeterPaul <- select(NTL.phys.nutrient.data.PeterPaul, lakename, year4, month, sampledate:po4) #overwrite; select all the columns but in a different order 
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#group_by so you don't have to do all the lakes individually; Are able to tell R what you want it to call the columns 
