...
#Specific to your own computer. Be careful about doing this. 
```
...
str(USGS.flow.data) #structure; i.e. summary of rows and columns 
dim(USGS.flow.data) #dimensions tells rows and columns 
length(USGS.flow.data) #tells you the number of columsn 

head(USGS.flow.data) #Give the first couple row of each of the columns 
head(USGS.flow.data, 10) #Can command to look at first 10 rows 
tail(USGS.flow.data, 5) #Can get the end of the data. Last five specified. 
USGS.flow.data[30000:30005, c(3, 8, 14)] #Can specify which rows and columns 

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) #Gives basic stats 
...
> ANSWER: They show N/A. Be carefull what class you import as. They treat NA vs. blanks differently. 
