...
#head(USGS.flow.data) #Will show first six lines of data 
...
format(today+4, format = "%y, %m, %d")
format(today, format = "%Y, %b, %d")
format(today, format = "%A,%m,%d,%B")
...
# formatting date as year, month, day in the datetime column 
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
# Create a function that tells it to change to 20 if the [day is not higher than 181231 - 2018, 31 December, that means we're somehwere in the 1900s] -- works because we changed the formate of datetime. Giving it something called "d" - the ingredient. pastzero will take the two statements inside and puts them together"
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#We've only written the fuction, noe we have to run it. 
...
> Answer: In social science there may be meaning to the NA (i.e. taking a survey); Be careful dealing with true NA versus missing data; Some statistical models cannot allow NA (for example, time series)

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #Operates on rows of data; Dollar signs and selecting certain columns are an option; #complete.cases tells you which cases are complete -- will quarry for full rows [na.omit will omit the entire row -- keeps only the days where all of the observations are complete]
dim(USGS.flow.data)
...
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean)) #choose data; provide aesthetics; automatically divided in to 30 bins but you can tell it a bin width 

#
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10) #dishcarge mean of 10 units - will give it in the units of whatever column that is 

#
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# Zooming in on some data for illustrative purposes 
...
```{r, fig.height = 3, fig.width = 4}
# More minimums are lower counts (blue); Created frequency polygons for three different columns
ggplot(USGS.flow.data.complete) +
...
# New measurements versus old measurements; Based on what categories they fall in to (from a single column); Color is based on the status of the other variable
ggplot(USGS.flow.data.complete) +
...
#gage height: the height of the river. The greater the depth of the river the more water is going to be flowing through. 
ggplot(USGS.flow.data.complete) +
...
#told it 3,4 to fit better on the page for a PDF 
```
