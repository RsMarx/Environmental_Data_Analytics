...
vector1[3] # Would like R to tell me the 3rd element of vector 1 -- 3rd number in list: matrix sub-setting uses the square brackets 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # use #s 1-20, make 5 rows and 4 columns. Moves vertically first and horizontally second (default) 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #will automatically do it by column unless we tell in byrow = TRUE
matrix2
...
matrix1[4, ] # Numbers in the 4th row 
matrix1[ , 3] #Numbers in the 3rd column
matrix1[c(12, 14)] # c tells it to take a few options. Wanted to put up elements 12 and 14
matrix1[c(12:14)] #Giving us elements 12, 13 and 14 (a section of elements) 
...
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # by rown and column, count by row
matrix4
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # Labeled the columns 

dataframe1[1:2] # showing columns 1 and 2; If it were matrix it would choose elements 1 and 2; A comma after would give us rows 1 and 2  
dataframe1[c("ID","Passed")] # 
dataframe1$Color # $ (dollar sign) gives you list of different elements 

```
QUESTION: How do the different types of data appear in the Environment tab?

> ANSWER: The types of data appear as lists of data and lists of values, if view is set to list rather than grid. Only things that have been defined will show up in the environments.

QUESTION: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> ANSWER: A list of elements to choose from appears 

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER: This tunrs it in to a section 
...
#install.packages(tidyverse) NOT FOUND 
library(tidyverse)
```
